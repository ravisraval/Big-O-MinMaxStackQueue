...
iterations = 1
Benchmark.bm do |bm|
  bm.report('bad_two_sum') { iterations.times { bad_two_sum?($arr, $target_sum) } }
  bm.report('two_sum') { iterations.times { two_sum?($arr, $target_sum) } }
  bm.report('okay_two_sum_b') { iterations.times { okay_two_sum_b?($arr, $target_sum) } }
  bm.report('okay_two_sum_a') { iterations.times { okay_two_sum_a?($arr, $target_sum) } }
  bm.report('four_sum') { iterations.times { four_sum?($arr, $target_sum) } }
end
